...
//请使用以下JSON测试增加功能（id为空）
    ...
    /**
     * POST, http://49.235.39.226:8080/department.ctl, 增加学院
    ...
    /**
     * DELETE, http://49.235.39.226:8080/department.ctl?id=1, 删除id=1的学院
    ...
    /**
     * PUT, http://49.235.39.226:8080/department.ctl, 修改学院
     *
...
//        response.setContentType("text/html;charset=UTF-8");
    ...
    /**
     * GET, http://49.235.39.226:8080/department.ctl?id=1, 查询id=1的学院
     * GET, http://49.235.39.226:8080/department.ctl, 查询所有的学院
     * 把一个或所有学院对象响应到前端
     * @param request
     * @param response
     * @throws ServletException
...
//        response.setContentType("text/html;charset=UTF-8");
        ...
        try {
            ...
            }else if ("school".equals(id_str1)){
                int id1= Integer.parseInt(id_str);
                responseFindAllBySchool(id1,response);
            }
            else {
                int id = Integer.parseInt(id_str);
    ...
    private void responseFindAllBySchool(int schoolId, HttpServletResponse response)
            throws ServletException, IOException, SQLException {
        //根据id查找学院
        Collection findAllBySchools = DepartmentService.getInstance().findAllBySchool(schoolId);
        String department_json = JSON.toJSONString(findAllBySchools);
